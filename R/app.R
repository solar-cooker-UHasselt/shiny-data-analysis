...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      mainPanel(
        strong("This script contains an R function to produce a dataset as output containing average measurements of, e.g., the
...
server <- function(input, output) {
  ...
  data1 <- reactive({
    ...
    output_dataset <- data.frame(
      Experiment_ID = rep(dat_long$Experiment_ID[1], 3),
      Pot_ID = 1:3
    )
    ...
    output_dat <- reshape(output_dataset,
      direction = "long",
      idvar = "Pot_ID",
      varying = list(
      ...
      v.names = c(
        "Temp_pot", "Outdoor_temp", "Wind_speed", "Air_pressure",
        "Rel_hum", "Solar_irr"
      ),
      timevar = "Time_period"
    )
